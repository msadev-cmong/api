...
sido = pd.read_csv('sido.csv')
sido = sido.where((pd.notnull(sido)), None)
accident = ["오토바이사고", "보행자", "운전자", "기타탈것", "자전거사고", "동승자"]
s_station = ["소방재난본부", "인천소방본부", "부산소방재난본부", "대구소방안전본부","대전소방본부", "광주소방본부", "울산소방본부","강원소방본부", "충북소방본부", "경남소방본부", "경북소방본부", "전남소방본부", "전북소방본부", "제주소방안전본부", "창원소방본부", "세종소방본부" ]
        ...
        for key, value in dict.items():
            for acc in accident:
                ...
                if key == "rlifAcdAsmCdNm" and value == acc:
                    p_count[s_station.index(nm)][accident.index(acc)] += dict["trnfPcnt"]

    url = url_im    

for i in range(len(s_station)):
    p_count[i].insert(0, s_station[i][0:2])

conn = pymysql.connect(host='35.194.13.180', port=3306, user='test', password='test', db='test_db', charset='utf8') 

cursor = conn.cursor()

sql_main =  'INSERT INTO main (시도, 이송환자수) VALUES(%s, %s)'

sql = 'INSERT INTO sub_table (시도, 오토바이, 운전자, 기타탈것, 자전거, 보행자, 동승자) VALUES (%s, %s, %s, %s, %s, %s, %s)'

for idx in range(len(sido)):
    cursor.execute(sql_main, tuple(sido.values[idx]))

for idx in range(len(p_count)):
    cursor.execute(sql, tuple(p_count[idx]))
